...
- Runtime coverage includes `.pi/bridge-outbox/`, `.pi/context-snapshots/`, `.pi/diagnostics/`, `.pi/mailbox/`, `.pi/runtime/`, `.pi/supervisor/`, `.pi/telemetry/`, and `.pi/verification/`. Project configuration and `.pi/agents/` remain trackable.
...
Doctor also reports missing runtime `.gitignore` entries and already-tracked runtime artifacts.
For projects initialized by an older version, add the missing directory entries listed above
to the existing `.gitignore`. Follow doctor's untracking hint
for artifacts already in the Git index; adding ignore rules alone does not untrack them.
