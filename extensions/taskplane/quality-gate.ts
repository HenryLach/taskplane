 ...
 *
 * Includes only blocking findings (critical + important) — suggestions are
 * omitted since they don't block the quality gate. The template is stable
 * across runs so fix-agent prompts are reproducible.
 *
...
export function generateFeedbackMd(
	...
	const lines: string[] = [
		...
		``,
		`> This file was generated by the quality gate. Address all critical and`,
		`> important findings below, then the review will re-run automatically.`,
		``,
	];
	...
	if (criticals.length === 0 && importants.length === 0 && mismatches.length === 0) {
		lines.push(`## No blocking findings`);
		lines.push(``);
		lines.push(`The review returned NEEDS_FIXES but no critical/important findings were extracted.`);
